...
<div class="relative">
          ...
          @if (isMenuOpen()) {
<div 
               class="absolute right-0 mt-3 w-64 bg-white rounded-3xl shadow-2xl border border-slate-100 p-2 z-[60] animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div class="space-y-1">
              @for (profile of auth.currentUser()?.profiles; track profile) {
<button 
                      (click)="switch(profile.id)"
