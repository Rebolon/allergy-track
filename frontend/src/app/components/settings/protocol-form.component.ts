    ...
    <section [ngClass]="theme.protocolSection()" class="space-y-6">
      @if (!onboardingMode()) {
        <div class="flex items-center gap-3 mb-2">
          ...
          <h3 class="text-xl font-black text-(--color-primary-focus)">Défis gourmands</h3>
        </div>
      }
      ...
      <form [formGroup]="form" (ngSubmit)="save()">
        ...
        <div class="relative overflow-hidden mb-6 p-4 rounded-2xl bg-white shadow-sm border border-slate-100/50 flex flex-col gap-2">
          <div class="absolute left-0 top-3 bottom-3 w-1.5 rounded-r-md bg-(--color-primary)"></div>
          ...
          <input type="date" formControlName="startDate" 
                 class="ml-3 p-3 bg-slate-50 rounded-xl border-none font-bold outline-none focus:ring-2 focus:ring-(--color-primary-focus) transition-all">
        </div>
Line 33: 
            ...
            <div [formGroupName]="i" class="relative overflow-hidden flex flex-col sm:flex-row items-center gap-4 p-4 rounded-2xl bg-white shadow-sm border border-slate-100/50">
              ...
              <div class="flex-1 w-full pl-3">
                ...
                <input type="text" formControlName="allergen" 
                       class="w-full p-3 font-bold bg-slate-50 rounded-xl border-none outline-none focus:ring-2 focus:ring-(--color-primary-focus)" 
                       placeholder="Ex: Noisette"
                       [title]="item.get('allergen')?.value || ''">
              </div>
              ...
              <div class="flex gap-3 w-full sm:w-auto">
                <div class="flex flex-col flex-1 sm:w-24">
                   ...
                   <input type="number" step="0.1" formControlName="dose" 
                          class="w-full p-3 font-bold bg-slate-50 rounded-xl border-none outline-none text-center focus:ring-2 focus:ring-(--color-primary-focus)" 
                          placeholder="0.0">
                </div>
                <div class="flex flex-col flex-1 sm:w-24">
        ...
        <button type="button" (click)="add()" 
                class="w-full py-4 mb-6 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 font-bold hover:border-(--color-primary) hover:text-(--color-primary-focus) transition-all flex items-center justify-center gap-2 bg-white/50">
          <lucide-icon [img]="Plus" [size]="18"></lucide-icon> 
          {{ onboardingMode() ? 'Ajouter un allergène' : 'Ajouter un défi' }}
        </button>

        <button type="submit" [disabled]="form.invalid || (!form.dirty && !onboardingMode())" 
                class="w-full py-4 bg-(--color-primary) text-white rounded-2xl font-black text-lg shadow-lg hover:bg-(--color-primary-focus) transition-all disabled:opacity-50 transform active:scale-95">
